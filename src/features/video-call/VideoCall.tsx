		...
		if (videoRef.current && stream && stream.active) {
			...
			videoRef.current.onloadedmetadata = () => {
				videoRef.current
					?.play()
					.then(() =>
						console.log("Video playback started for stream:", stream.id),
					)
					.catch((err: Error) =>
						console.error("Video play error for stream:", stream.id, err),
					);
			};
		}
	}, [stream]);
...
const VideoCall: React.FC = () => {
	...
	useEffect(() => {
		...
		return () => {
			console.log(
				"Cleaning up local stream, peer connections, and subscriptions",
			);
			if (localStream) {
				localStream
					.getTracks()
			...
			unsubscribeFunctions.current.forEach((unsubscribe: () => void) =>
				unsubscribe(),
			);
	...
	const joinRoom = async (roomId: string, stream: MediaStream) => {
		...
		try {
			...
			const unsubscribeParticipants: Unsubscribe = onSnapshot(
	...
	const setupIncomingConnectionListener = async (
		roomId: string,
		stream: MediaStream,
	) => {
		...
		const unsubscribeOffers: Unsubscribe = onSnapshot(
			offerQuery,
			(snapshot) => {
	...
	const createPeerConnection = async (
		...
		pc.ontrack = (event: RTCTrackEvent) => {
			...
			if (event.streams && event.streams[0]) {
				...
				console.log(
					"Setting remote stream for peer:",
					peerId,
					"stream ID:",
					remoteStream.id,
				);
	...
	const setupSignalingListeners = async (
		...
		const unsubscribeAnswers: Unsubscribe = onSnapshot(
			answerQuery,
			(snapshot) => {
				snapshot.docChanges().forEach(async (change) => {
					if (change.type === "added") {
		...
		const unsubscribeCandidates: Unsubscribe = onSnapshot(
			candidateQuery,
			(snapshot) => {
				snapshot.docChanges().forEach(async (change) => {
					if (change.type === "added") {
			...
			<div className="flex-1 flex flex-col">
				...
				<div className="bg-gray-800 text-gray-300 p-2 text-xs">
					<div className="flex justify-center space-x-4">
						...
						<span>
							Connections: {Object.keys(peerConnections.current).length}
						</span>
